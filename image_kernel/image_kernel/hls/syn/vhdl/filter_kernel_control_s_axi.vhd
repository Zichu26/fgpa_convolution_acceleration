...
entity filter_kernel_control_s_axi is
generic (
...
end entity filter_kernel_control_s_axi;
...
architecture behave of filter_kernel_control_s_axi is
    ...
    constant ADDR_FILTER_DIVISOR_DATA_0 : INTEGER := 16#10#;
    constant ADDR_FILTER_DIVISOR_CTRL   : INTEGER := 16#14#;
    ...
    constant ADDR_CHANNELS_DATA_0       : INTEGER := 16#28#;
    constant ADDR_CHANNELS_CTRL         : INTEGER := 16#2c#;
    constant ADDR_FILTER_BASE           : INTEGER := 16#40#;
    constant ADDR_FILTER_HIGH           : INTEGER := 16#7f#;
    ...
    signal int_filter_divisor  : UNSIGNED(31 downto 0) := (others => '0');
    ...
    component filter_kernel_control_s_axi_ram is
        generic (
    ...
    end component filter_kernel_control_s_axi_ram;
...
-- ----------------------- Instantiation------------------
-- int_filter
int_filter : filter_kernel_control_s_axi_ram
generic map (
     MEM_STYLE => "auto",
     MEM_TYPE  => "2P",
     BYTES     => 4,
...
-- ----------------------- AXI READ ----------------------
    ...
    RRESP   <= "00";  -- OKAY
    RVALID_t  <= '1' when (rstate = rddata) and (int_filter_read = '0') else '0';
    RVALID    <= RVALID_t;
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_FILTER_DIVISOR_DATA_0 =>
                        rdata_data <= RESIZE(int_filter_divisor(31 downto 0), 32);
...
-- ----------------------- Register logic ----------------
    ...
    filter_divisor       <= STD_LOGIC_VECTOR(int_filter_divisor);
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                int_filter_divisor(31 downto 0) <= (others => '0');
            elsif (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_FILTER_DIVISOR_DATA_0) then
                    int_filter_divisor(31 downto 0) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_filter_divisor(31 downto 0));
                end if;
            end if;
        end if;
    end process;
...
-- ----------------------- Memory logic ------------------
    ...
    int_filter_be1       <= UNSIGNED(WSTRB) when int_filter_we1 = '1' else (others=>'0');
...
entity filter_kernel_control_s_axi_ram is
    generic (
        MEM_STYLE : STRING :="auto";
        MEM_TYPE  : STRING :="S2P";
        BYTES   : INTEGER :=4;
...
end entity filter_kernel_control_s_axi_ram;

architecture behave of filter_kernel_control_s_axi_ram is
