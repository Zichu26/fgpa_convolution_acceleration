...
# 3 "filter_kernel.cpp" 2



typedef ap_uint<128> axie4_t;
...
#pragma HLS ARRAY_PARTITION variable=filter_local complete dim=0
 ...
 VITIS_LOOP_48_1: for (int i = 0; i < 3; i++) {
            VITIS_LOOP_49_2: for (int j = 0; j < 3; j++) {
#pragma HLS UNROLL
 ...
 filter_local[i][j] = filter[i][j];
        ...
        unsigned char line_buffer[3][1920][3];
...
#pragma HLS ARRAY_PARTITION variable=window complete dim=0
 ...
 init_line_buffer:
        for (int i = 0; i < 3; i++) {
            VITIS_LOOP_76_3: for (int j = 0; j < width; j++) {
                VITIS_LOOP_77_4: for (int k = 0; k < channels; k++) {
#pragma HLS PIPELINE
 line_buffer[i][j][k] = 0;
                }
            }
        ...
        for (int row = 0; row < height; row++) {
            ...
            for (int col = 0; col < width; col++) {
                ...
                VITIS_LOOP_89_5: for (int i = 0; i < 3; i++) {
                    VITIS_LOOP_90_6: for (int j = 0; j < 2; j++) {
                        VITIS_LOOP_91_7: for (int k = 0; k < channels; k++) {
#pragma HLS UNROLL
 window[i][j][k] = window[i][j+1][k];
                ...
                if (col == 0) {
                    shift_lines:
                    for (int i = 0; i < 2; i++) {
                        VITIS_LOOP_102_8: for (int j = 0; j < width; j++) {
                            VITIS_LOOP_103_9: for (int k = 0; k < channels; k++) {
#pragma HLS PIPELINE
 line_buffer[i][j][k] = line_buffer[i+1][j][k];
                ...
                if (row < height) {
                    ...
                    for (int j = 0; j < width; j += elements_per_axie4) {
                        ...
                        VITIS_LOOP_119_10: for (int x = 0; x < elements_per_axie4 && j + x < width; x++) {
                            VITIS_LOOP_120_11: for (int k = 0; k < channels && (j * channels + k) < elements_per_axie4; k++) {
#pragma HLS PIPELINE
 ...
 unsigned char pixel_val = (unsigned char) (axie4_data >> 8 * (x * channels + k));
                ...
                VITIS_LOOP_135_12: for (int i = 0; i < 3; i++) {
                    VITIS_LOOP_136_13: for (int k = 0; k < channels; k++) {
#pragma HLS UNROLL
 if (col < width - 1) {
                ...
                VITIS_LOOP_147_14: for (int k = 0; k < channels; k++) {
#pragma HLS UNROLL
...
#pragma HLS ARRAY_PARTITION variable=output_pixel complete dim=0
 ...
 VITIS_LOOP_160_15: for (int k = 0; k < channels; k++) {
#pragma HLS UNROLL
 float sum = 0.0f;

                        VITIS_LOOP_164_16: for (int i = 0; i < 3; i++) {
                            VITIS_LOOP_165_17: for (int j = 0; j < 3; j++) {
#pragma HLS UNROLL
 sum += (float)window[i][j][k] * filter_local[i][j];
                    ...
                    VITIS_LOOP_188_18: for (int k = 0; k < channels && (byte_offset + k) < elements_per_axie4; k++) {
#pragma HLS UNROLL
 ...
 axie4_data &= ~(axie4_t(0xFF) << (8 * (byte_offset + k)));
                    ...
                    if (byte_offset + channels > elements_per_axie4) {
                        ...
                        VITIS_LOOP_204_19: for (int k = 0; k < remaining_channels; k++) {
#pragma HLS UNROLL
 int channel_idx = elements_per_axie4 - byte_offset + k;
                    ...
                    VITIS_LOOP_222_20: for (int k = 0; k < channels && (byte_offset + k) < elements_per_axie4; k++) {
#pragma HLS UNROLL
 unsigned char pixel_val = (unsigned char)(input_axie4_data >> (8 * (byte_offset + k)));
                    ...
                    if (byte_offset + channels > elements_per_axie4) {
                        ...
                        VITIS_LOOP_237_21: for (int k = 0; k < remaining_channels; k++) {
#pragma HLS UNROLL
