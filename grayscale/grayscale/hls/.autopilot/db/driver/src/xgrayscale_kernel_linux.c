...
typedef struct {
    ...
    XGrayscale_kernel_uio_map maps[ MAX_UIO_MAPS ];
} XGrayscale_kernel_uio_info;

/***************** Variable Definitions **************************************/
static XGrayscale_kernel_uio_info uio_info;
...
static int uio_info_read_name(XGrayscale_kernel_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XGrayscale_kernel_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XGrayscale_kernel_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XGrayscale_kernel_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XGrayscale_kernel_Initialize(XGrayscale_kernel *InstancePtr, const char* InstanceName) {
	XGrayscale_kernel_uio_info *InfoPtr = &uio_info;
